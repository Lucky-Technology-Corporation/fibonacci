import { speechToText } from './rest-api.js';
...
class GoogleSpeechRecognition {
  ...
  async connectMicrophoneAndProcessor(callbackWhenSilent) {
    ...
    if (!this.processor) {
      ...
      this.processor.onaudioprocess = e => {
        ...
        if (speechDetected && isSilent) {
          if (performance.now() - silenceStart > pauseDuration) {
            callbackWhenSilent()
          }
        }
  ...
  async startListening(callbackWhenSilent) {
    if (this.starting) {
      // already starting
      return;
    }
    ...
    await this.connectMicrophoneAndProcessor(callbackWhenSilent);
