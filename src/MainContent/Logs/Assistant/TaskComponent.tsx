...
export default function TaskComponent({
  ...
  return (
    <div
      className={`flex flex-col bg-[#85869822] rounded px-4 p-2 mt-2 mb-2 w-full ${setPath && "cursor-pointer"}`}
      onClick={() => {
        if (setPath) {
    ...
    >
      <div className="flex items-center">
        <div className="w-full">
          <div className="flex text-sm items-center">{headerNode}</div>
          <div className="flex mt-2 w-full items-top">
            {isTaskComplete && (
              <FontAwesomeIcon icon={faCheckCircle} className="text-green-400 text-lg w-3 h-3 mr-1 my-auto" />
            )}
            ...
            <div className="no-focus-ring w-full">
              {isEditing ? (
                <input
                  ...
                  onChange={(e) => {
                    setTaskEditPrompt(e.target.value);
                  }}
                  onKeyDown={(event) => {
                    if (event.key == "Enter") {
                      editTaskSummary();
