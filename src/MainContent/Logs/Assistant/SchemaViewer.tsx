  ...
  const CollapsibleSection = ({ title, children, keyPath, collection, level = 0 }) => {
    ...
    return (
      <>
        <div
          key={title}
          className="flex justify-between align-middle my-2 p-1 rounded no-focus-ring"
          style={{ marginLeft: `${level * 20}px` }}
        >
          <div className="my-auto">
            <Button
              className={`${
                title == "_id" && "opacity-0 pointer-events-none"
              } w-3 ml-auto text-sm font-sans mr-2 py-1 font-medium rounded flex justify-center items-center cursor-pointer hover:bg-[#85869822]`}
              onClick={() => setIsOpen(!isOpen)}
              children={
                <FontAwesomeIcon
                  icon={isOpen ? faChevronDown : faChevronRight}
                  className="text-sm w-3 h-3 opacity-70 hover:opacity-100"
                />
              }
            />
          </div>
          ...
          <div
            className="my-auto ml-1 mr-auto cursor-pointer"
            onClick={() => {
              openAddModal(collection, keyPath);
            }}
          >
            <FontAwesomeIcon icon={faPlus} className="text-sm w-3 h-3 opacity-70 hover:opacity-100" />
          </div>
  ...
  const renderField = (field, type, collection, handleRemove, keyPath: string[], level = 0) => {
    if (typeof type === "object" && !Array.isArray(type)) {
      ...
      return (
        <div
          key={field}
          className="flex w-full justify-between align-middle my-2 p-1 px-0 rounded no-focus-ring"
          style={{ marginLeft: `${level * 20}px` }}
        >
          <div className="my-auto">
            <Button
              className={`${
                field == "_id" && "opacity-0 pointer-events-none"
              } w-3 ml-0 text-sm font-sans mr-2 py-1 font-medium rounded flex justify-center items-center cursor-pointer hover:bg-[#85869822]`}
              onClick={() => handleRemove(collection, keyPath)}
              children={
                <FontAwesomeIcon icon={faXmarkCircle} className="text-sm w-3 h-3 opacity-70 hover:opacity-100" />
              }
            />
          </div>
          <InputField
            key={keyPath.join("-") + "-" + field + "-input"}
            className={`my-auto w-full text-xs font-mono font-semibold ${
              field == "_id" ? "bg-transparent" : "bg-[#85869822]"
            } border-[#525363] rounded p-1 px-1 mx-0 mr-2`}
            initialValue={field}
          ...
          <div key={collection} className="my-2 mx-4 bg-[#85869822] p-2 rounded w-72">
            <div className="text-sm font-bold font-mono text-center my-1 no-focus-ring flex">
              ...
              <Button
                className={`${
                  true ? "hidden" : ""
                } w-4 ml-auto text-sm font-sans py-1 font-medium rounded flex justify-center items-center cursor-pointer`}
                onClick={() => {
            ...
            {Object.entries(fields).map(([field, type]) => renderField(field, type, collection, handleRemove, [field]))}
            <div
              className="my-auto mb-1 mr-auto cursor-pointer"
              onClick={() => {
                openAddModal(collection, []);
              }}
            >
              <FontAwesomeIcon icon={faPlus} className="text-sm w-4 h-4 opacity-70 hover:opacity-100" />
            </div>
          </div>
        ))}
      <NewFieldModal
        addFieldHandler={handleAdd}
