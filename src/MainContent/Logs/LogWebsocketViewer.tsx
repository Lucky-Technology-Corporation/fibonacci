...
export default function LogWebsocketViewer(props: LogWebsocketViewerProps) {
  ...
  const { testDomain, activeProject, activeEndpoint, setPostMessage, selectedTab, setFrontendRestarting } =
  ...
  useEffect(() => {
    if (props.injectedLog == "" || props.injectedLog == undefined) {
      return;
    }
  ...
  const greyOutUnimportantLines = (inputLine: string): ReactNode => {
    ...
    return inputLine.split("\n").map((line, index) => {
      if (line.trimStart().startsWith("at ")) {
        return (
          <span className="text-gray-500" key={"grey-" + index + "-" + timestamp}>
            {line}
            <br />
          </span>
        );
        ...
        return (
          <span className="text-green-500" key={"grey-" + index + "-" + timestamp}>
            {line}
            <br />
          </span>
        );
  ...
  const parseOutFilenamesAndCreateElement = (line: string, key: string) => {
          ...
          if (
            ...
            return (
              <span className="font-mono text-sm" key={key}>
                <span
                  className="text-purple-500 mr-2 cursor-pointer underline decoration-dotted"
                  onClick={autoFixImportJs("/backend/user-dependencies/" + fileName, line.split("):")[1], lineNumber)}
                >
                  [Autofix this issue]
                </span>
                <span className="text-red-500">
                  Error in {niceEndpoint.method} {niceEndpoint.fullPath} at{" "}
                  <span
                    onClick={() => {
                      setPostMessage({
                        type: "openFile",
        ...
        return (
          <span className="font-mono text-sm" key={key}>
            <span className="text-red-500">
              ...
              <span
                onClick={() => {
                  setPostMessage({ type: "openFile", fileName: "/" + page, line: lineNumber, column: 0 });
                }}
                className="cursor-pointer underline decoration-dotted"
              >
                line {lineNumber}
            ...
            {greyOutUnimportantLines(getStringAfterFirstNewline(line))}
          </span>
        );
      }
    }
    ...
    return (
      <span className="font-mono text-sm" id={key} key={key}>
        {greyOutUnimportantLines(cleanLine)}
      </span>
    );
  };
    ...
    if (errorText.includes("Did you mean '")) {
      ...
      return () => {
        ...
        setTimeout(() => {
          setPostMessage({
            type: "findAndReplace",
            findText: fixedImport.replace(".js", ""),
            replaceText: fixedImport,
          });
        }, 200);
        setTimeout(() => {
  ...
  const reconnectWebsocket = async (withLocation: string) => {
    ...
    const webSocket = new WebSocket(
      `wss://${testDomain.replace("https://", "fermat.")}/tail_logs?path=${path}&jwt=${fermatJwt}&initial_lines=25`,
    );
          ...
          allMessages.forEach((lineIn, index) => {
            try {
              ...
              for (var i = 0; i < lines.length; i++) {
                ...
                if (
                  currentLine.startsWith("Error: Cannot find module") &&
                  currentLine.includes("imported from /swizzle/code/server.ts")
                ) {
                  var fileName = currentLine
                    .split("Error: Cannot find module '")[1]
                    .split("' imported from /swizzle/code/server.ts")[0];
  ...
  return (
    <>
      <div
        ref={divRef}
        style={props.style}
        className="overflow-y-scroll border-t border-gray-700 py-1 mr-4 bg-[#1e1e1e]"
      >
        <div
          className="flex mt-1 z-40 fixed right-0 rounded mt-[-4px] p-1 bg-[#1e1e1e]"
          style={{ marginRight: getPosition() }}
        >
        ...
        <span className="font-mono text-sm">
          {log.map((entry, index) => {
            return (
              <div id={`parent_${index}`} key={`parent_${index}`}>
                {entry}
              </div>
            );
          })}
