...
import DatabaseEditorHint from "../Database/DatabaseEditorHint";
import DatabaseRow from "../Database/DatabaseRow";
import useApi from "../../API/DatabaseAPI";
import RowDetail from "../Database/RowDetail";
import Dropdown from "../../Utilities/Dropdown";
import DocumentJSON from "../Database/DocumentJSON";
    ...
    return (
        <div>
            ...
            <div className="flex">
                    ...
                    <thead className="bg-[#85869822]">
                        <tr className={`font-mono text-xs ${keys.length == 0 ? "hidden" : ""}`}>
                            ...
                            {keys.filter(k => k != "_deactivated").map((key, index) => (
                                <th className={`text-left py-1.5 ${index == (keys.length - 2) ? "rounded-tr-md" : ""}`} key={index+1}>{key == "_id" ? <>Swizzle UID (<a href="https://www.notion.so/Swizzle-e254b35ddef5441d920377fef3615eab?pvs=4" target="_blank" rel="nofollow" className="underline decoration-dotted text-[#d2d3e0] hover:text-white">docs</a>)</> : key}</th>
                            ))}
                        </tr>
                    </thead>
                ...
                <DocumentJSON document={jsonEditorData} collection={activeCollection} isVisible={isJSONEditorVisible} setIsVisible={setIsJSONEditorVisible} onChange={onJSONChangeHandler} />                                       
            </div>
            {data.length == 0 && (
                <div className="flex-grow flex flex-col items-center justify-center">
                    <div className="text-lg font-bold mt-4 mb-4">😟 No users yet</div>
                </div>
            )}
        </div>
    )
