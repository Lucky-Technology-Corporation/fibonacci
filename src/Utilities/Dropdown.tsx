...
export default function Dropdown({children, lastChild, onSelect, lastOnSelect, className, direction = "left", title}: DropdownProps) {
    ...
    return (
        ...
        <Menu.Button className="inline-flex w-full justify-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold shadow-sm bg-[#85869833] ring-1 ring-inset ring-[#525363]">
            {title ? title : (children.find((child: any) => child.id == selected) || {"name": "Loading..."}).name}
        ...
        <Menu.Items className={`absolute ${direction}-0 z-50 mt-2 w-56 origin-top-right rounded-md shadow-lg bg-[#32333b] ring-1 ring-inset ring-[#525363] focus:outline-none`}>
                ...
                {children.map((child: any) => (
                    ...
                    {({ active }) => (
                        <a
                            ...
                            onClick={() => { onSelect(child.id); setSelected(child.id)}}>
                         {child.name}
                     </a>
                    )}
                    </Menu.Item>
            ...
            {lastChild && (
                <Menu.Item key={lastChild.id}>
                    ...
                    <a
                        href="#"
                        onClick={() => { if(lastOnSelect) { lastOnSelect(lastChild.id) }}}
                        className={classNames(
                        active ? '' : 'text-[#D9D9D9] ',
