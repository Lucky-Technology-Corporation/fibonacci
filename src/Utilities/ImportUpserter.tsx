  ...
  try {
    ...
    imports.forEach((imp) => {
      ...
      if (existingDeclaration) {
        if (imp.named) {
          const specifierExists = existingDeclaration.specifiers.some(
            (specifier) =>
              t.isImportSpecifier(specifier) &&
              t.isIdentifier(specifier.imported) &&
              specifier.imported.name === imp.import,
          );
          if (!specifierExists) {
        ...
        const newImportDeclaration = imp.named
          ? t.importDeclaration(
              [t.importSpecifier(t.identifier(imp.import), t.identifier(imp.import))],
              t.stringLiteral(imp.from),
            )
        ...
        if (!importStatement.includes(imports[i].import)) {
          if (imports[i].named) {
            if (importStatement.includes("}")) {
              ...
              newImportStatement =
                importStatementParts[0] + ", { " + imports[i].import + " } from " + importStatementParts[1];
            }
