        ...
        <div className={`flex-col items-center justify-between ${show ? "opacity-100" : "opacity-0 h-0"}`} style={{transition: "opacity 0.3s"}}>
            ...
            <InfoItem
                ...
                toast={{
                    title: "response",
                    content: (
                        <div className='text-gray-400'>Send your response with the following syntax:
                            {getTableHelper([
                                {name: "response.send({})", description: "Send an object back to the frontend"},
                                {name: "response.status(400).send({})", description: "Define the status code sent back to the frontend"},
                                {name: "response.redirect(url)", description: "Redirect to another url"}
                            ])}
                        </div>
                    )
                }}
