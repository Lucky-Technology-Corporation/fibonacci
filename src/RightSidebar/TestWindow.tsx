...
if (showNewTestWindow) {
  ...
  return (
    ...
    <img src="playbutton.svg" alt="playbutton" className="w-4 h-4 mr-2 mt-2"/>
</button>


            <Button text={testDoc.test_name} onClick={() => {setShowNewTestWindow(true); setTestDoc(testDoc)}} className = "mt-3 w-full inline-flex justify-center rounded-md border border-gray-600 shadow-sm px-4 py-2 bg-[#32333b] text-base font-medium text-[#D9D9D9] hover:bg-[#525363]  sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"/>
            ...
            {testResults[testDoc._id] !== undefined && (
                <>
                    ...
                    <Dot className="ml-2" color={getColorByStatus(testResults[testDoc._id])} />
                </>
            )}
        </div>
        </div>
    ...
    icon={faTrash}
      ...
      <div className="flex justify-end items-center pb-4 mr-4 space-x-4">
        ...
        <Button text="Run All" onClick={runAllTests} className="mt-2 inline-flex justify-center rounded-md border border-gray-600 shadow-sm px-4 py-2 bg-[#44464f] text-base font-medium text-white hover:bg-[#525363]  sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" />
        </div>
    </div>
  );
}
